...
################################################################
async def rhoaiNotebooksScaleMapper(start_datetime: date, end_datetime: date, configpath: str):
    df = await getData(start_datetime, end_datetime, configpath)
    df.insert(len(df.columns), "product", "RHOAI Notebooks Scale")
    ...
    df["testName"] = df["metadata.settings.test_case"].fillna("no test")
