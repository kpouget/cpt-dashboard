...
import {BASE_URL, OCP_GRAPH_API_V1, OCP_JOBS_API_V1, CPT_JOBS_API_V1, QUAY_JOBS_API_V1, QUAY_GRAPH_API_V1, RHOAI_NOTEBOOKS_PERF_API_V1} from "../Shared";
...
export const fetchRHOAINotebooksPerfJobsData = (startDate = '', endDate='') => async dispatch => {
    let buildUrl = `${BASE_URL}${RHOAI_NOTEBOOKS_PERF_API_V1}`
    dispatch(setWaitForOCPUpdate({waitForUpdate:true}))
    if (startDate !== '' && endDate !== '') {
        buildUrl += `&start_date=${startDate}&end_date=${endDate}`
    }
